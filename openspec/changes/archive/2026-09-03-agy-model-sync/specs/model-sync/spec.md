...
Effort variants (e.g. `high`, `medium`, `low`, `thinking`) MUST be derived dynamically from the slugs actually exposed by `agy` at sync time, not from a static hardcoded list. The behavior MUST remain identical to the current hardcoded flow for existing models.
...
- THEN it MUST produce the same 7 bases and variant sets as today (e.g. `gemini-3.7-flash -> {high,medium,low}`, `claude-sonnet-4-6 -> {}` singleton)
