...
Each model MUST expose a `variants` map for effort (subset per base, e.g. `{high,medium,low}` for `gemini-3.7-flash`, `{high,low}` for `gemini-3.1-pro`, `{}` for singleton `claude-sonnet-4-6`). Variants are ONLY the effort UX; no flat `-high` model ids.
